...
WANTED = ("MAX_GROUP", "MAX_TRAFFIC", "MAX_ACTIVATION_MS", "MIN_SILENCE_MS", "DAILY_BUDGET_MS",
...
<div class="ctl">
  ...
  <label>Vehicles in the zone <b><span id="vehTxt">0</span> on the road</b></label>
  <input type="range" id="veh" min="0" max="9" step="1" value="0">
  <div class="zones"><span>0</span><span>limit is {maxtraffic}</span><span>9</span></div>
  <div class="presets" id="vPresets">
    <span class="chip" data-v="0">0 &mdash; clear road</span>
    <span class="chip" data-v="6">6 &mdash; busy, refuses</span>
  </div>
...
// ---- inputs
const dist = el("dist"), grp = el("grp"), veh = el("veh");
...
for (const c of document.querySelectorAll("#vPresets .chip"))
  c.addEventListener("click", () => {{ veh.value = c.dataset.v; }});
}});

el("bStop").addEventListener("click", () => {{ S.estop = true; }});
el("bReset").addEventListener("click", () => {{
  boot(false);
  dist.value = SEEN_MAX; grp.value = 1; veh.value = 0;
...
function tick() {{
  ...
  const traffic = +veh.value;

  el("cmTxt").textContent = cm;
  el("vehTxt").textContent = veh.value;
  ...
  if (S.doNotEmit)                  deny = "animal is on the do-not-emit list";
                                       ...
                                       farRefusal = true; }}
  else if (traffic > K.MAX_TRAFFIC) deny = "traffic in the zone - a startled animal could bolt into it";
...
let sent = {{d: null, g: null, v: null, p: false, n: false, e: false}};
let lastSend = 0;
...
async function linkBoard() {{
  ...
  try {{
    ...
    sent = {{d: null, g: null, v: null, p: false, n: false, e: false}};
    txt("r");
  }} catch (e) {{
...
function mirror(cm, group) {{
  ...
  if (traffic !== sent.v)   {{ sent.v = traffic;   txt("v" + traffic); }}
...
const DEMO = [
  ...
  [12000, "An animal approaches",
   ...
   + "flash is a fresh decision.", "approach"],
  [5200, "A person is in the cone",
   "Absolute veto. Emission stops mid-burst, and it says why.", "person"],
  [5200, "A dog or a goat is in the cone",
   "The second absolute veto. Frequency gives no species selectivity - cattle "
   + "rank sixth of seven audiograms - so the selectivity has to come from the "
   + "decision, not the waveform.", "nontarget"],
  [7000, "The person leaves",
   "Clear again, so it goes straight back to permitting.", "leaves"],
  [5200, "A herd, not one animal",
   "Five together. A startle can cascade into the road, so it refuses.", "herd"],
  [6000, "Traffic in the zone",
   "Six vehicles on the road with the animal. This is the rule that looks "
   + "backwards, and it is the most important one: the emitter startles, it "
   + "does not steer. Startling an animal next to moving traffic can cause "
   + "the collision the system exists to prevent, so it refuses and warns "
   + "the traffic instead.", "traffic"],
  [5200, "Already at the carriageway",
   "Too close. Fleeing from here would mean crossing the road.", "carriageway"],
  [5000, "Beyond the envelope",
   "Out of range - the yellow lamp. It will not pretend to act at a distance "
   + "it cannot reach.", "far"],
  [6000, "The enforced quiet period",
   "It has just emitted, so it will not emit again yet. The full silence is "
   + "owed before the next burst, whatever the animal does.", "quiet"],
  [9000, "It gives up",
   "Three attempts, then it stops and asks for a human. Acoustic cues are "
   + "not stock-proof and this does not pretend otherwise.", "escalate"],
...
function boardToRest() {{
  ...
  txt("g1");
  sent = {{d: 0, g: 1, v: 0, p: false, n: false, e: false}};
}}
...
function demoBeat(i) {{
  if (i >= DEMO.length) {{
    boot(false);
    dist.value = SEEN_MAX; grp.value = 1; veh.value = 0; person = false; nonTarget = false;
    el("bPerson").classList.remove("held");
    demoStop("That is the whole argument: it detects, and then it declines.");
    return;
  }}
  demoAt = i;
  const [ms, head, script, mode] = DEMO[i];  // not 'say': that is the logger
  clearInterval(pulseTimer);
  pulseTimer = null;
  ...
  /* Keyed on the beat's name, never on its index.
   * An earlier version matched on i, so inserting a beat in the middle
   * silently reassigned every later one - the E-stop beat became the quiet
   * period and nothing announced the swap. A beat now carries its own world. */
  const W = {{
    clear:       {{ d: SEEN_MAX, g: 1 }},
    approach:    {{ d: 25,       g: 1 }},
    person:      {{ d: 25,       g: 1, person: true }},
    nontarget:   {{ d: 25,       g: 1, nonTarget: true }},
    leaves:      {{ d: 25,       g: 1 }},
    herd:        {{ d: 25,       g: 5 }},
    traffic:     {{ d: 25,       g: 1, v: 6 }},
    carriageway: {{ d: 8,        g: 1 }},
    far:         {{ d: 55,       g: 1 }},
    quiet:       {{ d: 25,       g: 1, cooldown: true }},
    escalate:    {{ d: 25,       g: 1 }},
    estop:       {{ d: 25,       g: 1, estop: true }},
  }};
  const w = W[mode];
  if (!w) {{ say("m", "demo: beat " + i + " has no world named " + mode); return; }}

  if (mode === "clear") boot(true);
  dist.value = w.d;
  grp.value  = w.g;
  veh.value  = w.v || 0;
  if (w.person)    {{ person = true;    el("bPerson").classList.add("held"); }}
  if (w.nonTarget) {{ nonTarget = true; el("bNon").classList.add("held"); }}
  if (w.estop)     S.estop = true;
  if (w.cooldown)  {{
    /* Land just past an emission, so the quiet period is what refuses here
     * rather than something the run arrives at by accident. */
    S.state = "COOLDOWN";
    S.lastEmissionEnd = now();
    S.attempts = 1;
    S.incidentStart = now();
